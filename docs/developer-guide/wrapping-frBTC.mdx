...
sidebar_label: frBTC Wrapping
sidebar_position: 3
---

# frBTC Wrapping & Unwrapping

frBTC is a synthetic Bitcoin token pegged 1:1 to BTC. This guide explains how wrapping and unwrapping works at the protocol level.
...
The SUBFROST signer address is derived from the frBTC contract:

```bash
alkanes brc20-prog signer-address
```

This queries opcode 103 (GET_SIGNER) on the frBTC contract and returns the Taproot address derived from the signer's public key.
...
- [frBTC Smart Contracts](./frBTC-smart-contracts) — Contract opcodes and methods
