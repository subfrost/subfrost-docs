...
:::info EASIEST WAY TO INTEGRATE
In our ALPHA phase, we encourage partners to simply integrate SUBFROST's wrap/unwrap via the [oyl-sdk](https://github.com/Oyl-Wallet/oyl-sdk).
We have created an .md file that can be utilized by your team or LLM of choice. [Let's Integrate.](pathname:///subfrost-integration.md)
:::

*You can ignore below if you are utilizing the oyl-sdk.*
...
With `subfrost-node` and `subrail-cli` installed, you are now ready to run a node and interact with the network.

**Continue to:**
- [Alkanes Integration](./subfrost-cli) — CLI commands for Alkanes operations (wrapping, unwrapping, token transfers)
- [BRC2.0 Integration](./brc20-prog) — Deploy and call EVM-compatible contracts on Bitcoin
- [frBTC - Alkanes](./wrapping-frBTC) — Detailed guide on wrapping/unwrapping frBTC
- [FR-BTC - BRC2.0](./frBTC-brc20) — Building scripts with FR-BTC on BRC2.0