...
This guide will walk you through the process of setting up the necessary tools to interact with the SUBFROST network. You will install the `subfrost-node` to connect to the network and the `subrail-cli` CLI for user-level interactions like managing assets.
