...
A keystore is a file that contains the cryptographic keys for a SUBFROST signing group. It is used by the `deezel` CLI to sign transactions on behalf of the group.

## Generating a Keystore

You can generate a new keystore using the `deezel` CLI. The following command will create a new keystore file with a 2-of-3 threshold signature scheme:

```bash
deezel frost create --threshold 2 --max-signers 3 --network testnet > keystore.json
```
