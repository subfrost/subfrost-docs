...
**Flexible Round-Optimized Schnorr Threshold (FROST)** and **Robust Asynchronous Schnorr Threshold (ROAST)** signatures are the cryptographic cornerstones of the SUBFROST protocol. They are threshold signature schemes that allow a large, dynamic set of signers to collectively create a single Schnorr signature without any single signer ever possessing the full private key.
