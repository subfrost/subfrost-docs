...
*   **ROAST:** Round-Optimized Schnorr Threshold. An optimization of FROST that allows for a non-interactive signing session, assuming a single round of pre-computation has already been performed.
...
*   **`dxBTC`:** A yield-generating synthetic Bitcoin asset built on top of the Subfrost ecosystem.
*   **`FROST` Token:** The governance and utility token of the Subfrost ecosystem.
...
*   **`SubfrostNetworkBehaviour`:** A composite `libp2p` `NetworkBehaviour` that combines all the different protocols and behaviours used by Subfrost.
*   **`gossipsub`:** A pub/sub (publish/subscribe) protocol where peers can subscribe to topics and receive messages that are published to those topics.
