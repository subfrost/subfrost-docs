...
- **Wrap**: Convert BTC → frBTC on Alkanes
- **Unwrap**: Convert frBTC → BTC via FROST threshold signatures
...
**Flow Diagram**:
```
User → frBTC.unwrap(amount, btc_addr)
         ↓
    Burn frBTC
         ↓
...
```
User swaps frBTC ↔ DIESEL in AMM pool
...
✅ **Deterministic Minting**: frBTC minted exactly matches BTC sent (minus premium)  
✅ **No Intermediaries**: Direct contract interaction  
✅ **Verifiable State**: All minting on-chain and reproducible  

### Unwrapping Security

✅ **Threshold Signatures**: M-of-N validators required  
✅ **No Single Point of Failure**: Distributed key shares  
✅ **Bitcoin L1 Settlement**: Direct to native BTC  
✅ **Transparent Process**: All transactions visible on-chain  
...
✅ **Bounded Range**: Premium limited to 0-100%  
✅ **Transparent**: Premium rate queryable on-chain  
⚠️ **Centralization**: Owner can adjust premium (mitigated by transparency)
...
- [Wrap Your First BTC →](/tokens/wrapping-frbtc)
- [Understand ftrBTC Options →](/tokens/ftrbtc-overview)
- [Explore DeFi Vaults →](/tokens/yield-vaults)
- [Use the API →](/subfrost-api/alkanes-methods)
