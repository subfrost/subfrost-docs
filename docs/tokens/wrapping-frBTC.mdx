---
title: frBTC - Wrapping/Unwrapping
sidebar_position: 2
---

# Wrapping & Unwrapping frBTC

As you have read by now, one of the core functionalities of the SUBFROST protocol is the ability to create `frBTC`, a synthetic asset pegged 1:1 to Bitcoin.
...
Note: Platforms that integrate wrap/unwrap may charge an additional platform fee.

### 1. Generate a SUBFROST Wallet

For technical users, if you haven't already, you need a SUBFROST wallet to receive your `frBTC`.
...
After the Bitcoin transaction is confirmed, the frBTC will be minted on the target protocol. You can check your balance using `subrail-cli`.
...
Unwrapping is the reverse process: `frBTC` is burned (or, melted!) and the equivalent amount of BTC is released from the SUBFROST multisig address back to a Bitcoin address you specify.
...
Once the Unwrap transaction is finalized, the SUBFROST signers will cooperatively sign a Bitcoin transaction to release the locked BTC to your specified destination address. This process is automated and secured by the SUBFROST Protocol.