...
*   **fuel:** The compute budget an Alkanes contract call is allowed to spend, comparable to gas on Ethereum in what it measures. Unlike gas, it is free: each block shares a fixed fuel budget among its transactions by size, and nothing is charged against it. Network fees are the ordinary Bitcoin miner fee, in native BTC.
