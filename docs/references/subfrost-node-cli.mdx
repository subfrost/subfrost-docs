...
This page provides a complete reference for all commands, subcommands, and flags available in the `subfrost-node` command-line interface. The `subfrost-node` is the core application for running a validator or full node on the SUBFROST network.
...
-   `--validator`: Run the node in validator mode. This requires a key for signing blocks.
...
A utility command to generate a new secret key for a validator.
