...
与 gas 的相似之处仅止于"计算预算"这一点。与 gas 不同，**fuel 是免费的**：你无需购买，无法充值，也不会有任何费用从中扣除。每个区块的固定 fuel 预算按交易大小在各笔交易之间分配。你为交易所支付的唯一费用，是普通的 Bitcoin 矿工费，以**原生 BTC** 支付，与一笔完全不运行合约的交易完全相同。详见 [Alkanes](../protocol/alkanes)。
:::
