...
不存在 fuel 价格，不存在 fuel 市场，也没有需要充值的余额。每个区块自带一份固定的 fuel 预算，区块中的每笔交易按其字节大小的比例分得其中一份，并设有下限，使得即使是小额交易也能完成有意义的工作。交易未用完的 fuel 会归还给区块，供下一笔交易使用。整个过程中不会以任何资产收取任何费用。
...
协议手续费与网络手续费是两回事：包装与解包 BTC 收取 0.1%，交换则收取资金池手续费。它们以所涉及的资产计费，与 fuel 无关。
