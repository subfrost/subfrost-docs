...
投票权是**按地址**读取的，而放在金库里的 FIRE 不在你的地址上。头寸正是为此存在：它持有这笔存款，另有一个单独的代币承载投票权。

一次向 cFIRE 金库的 FIRE 存入会产生两样东西：
...
委托背后没有注册表，也没有签名方案。投票代币是**持票人（bearer）**资产，谁持有它，谁就用它投票。所以委托就是一次普通的转账，只不过我们为它做了一个界面。
...
:::warning[发送 cFIRE-POS 就是把你的 FIRE 送走]
谁持有头寸（**cFIRE-POS**），谁就能退出它，所以发送头寸等于发送这笔存款。发送投票代币（**cFIRE-DELEGATE**）只发送投票权。**除非你真的想把自己的 FIRE 送走，否则绝不要发送 cFIRE-POS NFT。** 唯一用来传递的 NFT 是 **cFIRE-DELEGATE**。
:::
