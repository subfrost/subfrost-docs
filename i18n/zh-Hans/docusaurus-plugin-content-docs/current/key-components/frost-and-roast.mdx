...
Bitcoin 与 `frBTC` 之间的 1:1 锚定由大量签名者组成的池来维护。用于抵押 `frBTC` 的 BTC 存放在一个单一的 Bitcoin 地址中，其私钥由整个签名者集合通过 FROST 协议管理。请阅读[**此处**](/tokens/frBTC-overview#frbtc-路线图)了解我们签名组的两阶段方案。
